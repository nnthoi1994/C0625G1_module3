...
VALUES ('Laptop Dell XPS', 'Laptop i7 đời 2020', 15000000, 'DANG_CAM'),
       ('Xe máy Vision', 'Honda Vision 2019', 20000000, 'DANG_CAM'),
       ('Iphone 13', '128GB màu đen', 18000000, 'DA_CHUOC'),
       ('Nhẫn vàng 18K', 'Trang sức vàng', 10000000, 'THANH_LY'),
       ('Máy ảnh Canon', 'Canon 700D + lens 50mm', 12000000, 'DANG_CAM'),
       ('Tivi Samsung 55 inch', 'Smart TV 4K', 14000000, 'DANG_CAM'),
       ('Laptop Macbook Pro', 'Macbook Pro M1 2021', 25000000, 'DANG_CAM'),
       ('Xe AirBlade', 'Honda AirBlade 2020', 30000000, 'DANG_CAM'),
       ('Điện thoại Oppo Reno6', 'Oppo chính hãng', 8000000, 'DA_CHUOC'),
       ('Vàng 24K', 'Dây chuyền vàng 24K', 22000000, 'THANH_LY'),
       ('Máy giặt LG', 'Máy giặt LG Inverter 9kg', 7000000, 'DANG_CAM'),
       ('Tủ lạnh Toshiba', 'Tủ lạnh 300L', 9000000, 'DANG_CAM'),
       ('Điện thoại Samsung S21', 'Samsung S21 256GB', 20000000, 'DANG_CAM'),
       ('Xe máy Exciter', 'Yamaha Exciter 150', 28000000, 'DANG_CAM'),
       ('Laptop HP Envy', 'Laptop HP đời 2021', 16000000, 'DANG_CAM'),
       ('Iphone 12', '128GB màu trắng', 15000000, 'DA_CHUOC'),
       ('Nhẫn bạc 925', 'Trang sức bạc', 5000000, 'THANH_LY'),
       ('Máy ảnh Nikon', 'Nikon D5600 + lens', 13000000, 'DANG_CAM'),
       ('Tivi LG 50 inch', 'Smart TV 4K', 12000000, 'DANG_CAM'),
       ('Laptop Asus ROG', 'Laptop chơi game', 30000000, 'DANG_CAM'),
       ('Xe máy Lead', 'Honda Lead 2020', 18000000, 'DANG_CAM'),
       ('Điện thoại Xiaomi Mi 11', '128GB', 9000000, 'DA_CHUOC'),
       ('Vàng 18K', 'Lắc vàng 18K', 20000000, 'THANH_LY'),
       ('Máy giặt Electrolux', '9kg Inverter', 8000000, 'DANG_CAM'),
       ('Tủ lạnh Panasonic', '300L', 10000000, 'DANG_CAM'),
       ('Laptop Lenovo', 'Core i5 2020', 12000000, 'DANG_CAM'),
       ('Xe máy Winner', 'Honda Winner X', 25000000, 'DANG_CAM'),
       ('Iphone 11', '128GB', 13000000, 'DA_CHUOC'),
       ('Nhẫn vàng 24K', 'Vàng nữ trang', 25000000, 'THANH_LY'),
       ('Máy ảnh Sony', 'Sony Alpha 7', 22000000, 'DANG_CAM'),
       ('Tivi Sony 55 inch', 'Smart TV', 18000000, 'DANG_CAM');
-- ==========================
...
SELECT pc.pawn_contract_id, c.full_name, e.full_name, p.product_name
...
SELECT pc.pawn_contract_id, c.full_name AS customer_name, e.full_name AS employee_name,
       ...
       CASE
           WHEN pc.return_date IS NOT NULL THEN 'CLOSED'
...
SELECT customer_id, full_name, citizen_number, phone_number FROM customer
WHERE phone_number = '151617181920' OR citizen_number = '151617181920';


SELECT * FROM pawn_contract;
SELECT pc.pawn_contract_id, pc.pawn_date, pc.pawn_price, pc.interest_rate, 
       pc.due_date, pc.return_date,
       c.full_name AS customer_name, c.email AS customer_email, c.dob AS customer_dob, c.citizen_number AS customer_cccd,
       e.full_name AS employee_name,
       p.product_name, p.description AS product_description, p.pawn_price AS product_value
FROM pawn_contract pc
JOIN customer c ON pc.customer_id = c.customer_id
JOIN employee e ON pc.employee_id = e.employee_id
JOIN product p ON pc.product_id = p.product_id
WHERE pc.pawn_contract_id = 32;
...
FROM
    product p
JOIN
    pawn_contract pc ON p.product_id = pc.product_id
WHERE
    pc.due_date < CURDATE()
